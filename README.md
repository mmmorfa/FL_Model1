...
## Additional Details about the Software

### Frameworks and Libraries
- The system is developed in Python and utilizes specialized libraries such as:
  - **Stable Baselines3**: For implementing DQN models.
  - **Flower**: For orchestrating federated learning processes.

### DQN Models
- The implementation of Models 1 and 2 leverages the `DQN` class from Stable Baselines3.
- Models are fine-tuned with specific hyperparameters to optimize performance for:
  - **Model 1**: Admission control of slices.
  - **Model 2**: Maintaining QoS for processed slices.

### Flower Integration
- The `DQNClient` class in `DQN_client_fw.py` extends Flower's capabilities by integrating DQN models into the federated learning process.
- It facilitates seamless communication and synchronization between the server and clients.

### Custom Environments
- The simulation environments are defined in the `gym_examples` directory.
- These custom environments replicate realistic scenarios of a smart city, allowing accurate evaluation of models under diverse conditions.

### Secure Communication
- Communication between the server and clients is secured using TLS.
- Certificates for this secure communication are generated by the `certificates_generator.sh` script, ensuring data confidentiality and integrity.

### Evaluation and Visualization
- Scripts for performance evaluation include:
  - `agent_evaluate_individual.py`: Evaluates individual DQN models trained through FL.
  - `agent_evaluate_system.py`: Evaluates the system at a single node level, including Model 1, Model 2, and their interactions with the global database.
- Generated graphs provide insights into key metrics such as rewards obtained and success rates, aiding in system behavior analysis.

### Logging and Analysis
- During training, clients generate detailed logs stored in the `logs` directory.
- The `parse_logs.py` script analyzes these logs, creating visualizations to better understand the learning process and fine-tune parameters.

### Configuration and Deployment
- The `setup.py` script simplifies dependency installation, ensuring a consistent runtime environment.
- Pre-configured scripts such as `clientX_Model1.py` and `server.py` enable rapid deployment of client nodes and the server in the simulation environment.
